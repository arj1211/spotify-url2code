...
global cookie_policy_closed
cookie_policy_closed = False
...
def get_elem(key_name):
    ...
    if "index" in elem.keys():
        ...
        WebDriverWait(driver, 60).until(EC.visibility_of(elem))
...
def input_flow(url_to_input: str, _DEBUGGING=False):
    ...
    # ).perform()
    global cookie_policy_closed
    if not cookie_policy_closed:
        get_elem("cookie_policy_close").click()
        cookie_policy_closed = True
